...
parser.add_argument('--niter', type=int, default=8, help='number of epochs to train for')
parser.add_argument('--out_class', type=int, default=62338, help='number of classes')
parser.add_argument('--lr', type=float, default=0.01, help='learning rate, default=0.01')
parser.add_argument('--fea_lr', type=float, default=0.001, help='other module (not include fc) learning rate, default=0.01')
parser.add_argument('--cls_lr', type=float, default=0.01, help='fc learning rate, default=0.01')
...
#googlenet.classifier = AngleLinear(num_ftrs,opt.out_class)
...
all_paras = filter(lambda p: (p.requires_grad),googlenet.module.parameters())
cls_paras = list(googlenet.module.classifier.parameters())
...
for epoch in range(opt.niter):
    ...
    for i, (data,label) in enumerate(dataloader,0):
        ...
        if (i+1)%opt.log_step == 0:
            accuracy = compute_accuracy(out[0],labelv).data[0]           
